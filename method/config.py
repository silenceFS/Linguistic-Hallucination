...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        # post processing
        self.parser.add_argument("--hallucination_weight", type=float, default=0.01, help="balance weight of hallucination loss")
        self.parser.add_argument("--consistency_weight", type=float, default=0.2, help="balance weight of consistency loss")
        self.parser.add_argument("--support_set_number", type=int, default=3,help = 'the size of support set')
        self.parser.add_argument("--train_support", action="store_true", help='retrain support model or use a pretrained one')
        self.parser.add_argument("--load_support_model", action="store_true", help='load pretrained support model or not')
        self.parser.add_argument("--curriculum", action="store_true", help='curriculum learning or not')
        self.parser.add_argument("--support_ckpt_filepath", default='',help = 'pretrained stage1 model path')
        self.parser.add_argument("--model_name", type=str, default='MS_SL_Net')
        self.parser.add_argument('--root_path', type=str, default='',help='path of dataset')
    ...
    def parse(self):
        ...
        if isinstance(self, TestOptions):
            ...
            opt.results_dir = os.path.join(opt.root_path, opt.collection,
                                           "results", "-".join([opt.dset_name, opt.exp_id,time.strftime("%Y_%m_%d_%H_%M_%S")]))
