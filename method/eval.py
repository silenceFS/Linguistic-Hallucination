...
def compute_context_info(model, eval_dataset, opt, stage):
    model.eval()
    ...
    for idx, batch in tqdm(enumerate(context_dataloader), desc="Computing query2video scores",
        ...
        if stage == 'support':
            ...
            _frame_feat, _video_proposal_feat,  = model.encode_context_generate(clip_video_feat_, frame_video_feat_, frame_mask_)
...
def compute_query2ctx_info(model, eval_dataset, opt, ctx_info, stage):
    model.eval()
    ...
    for idx, batch in tqdm(enumerate(query_eval_loader), desc="Computing q embedding", total=len(query_eval_loader)):
        ...
        support_set_tokens = batch[8].to(opt.device)
        support_set_tokens_mask = batch[9].to(opt.device)
        support_set_tokens_padding_mask = batch[10].to(opt.device)
        if stage == 'support':
            support_query_feat = batch[4].to(opt.device)
            support_query_mask = batch[5].to(opt.device)
            support_mask = batch[6].to(opt.device)
            ...
            query = model.encode_query(cat_query_feat, cat_query_mask)

            _clip_scale_scores, _frame_scale_scores = model.get_pred_from_raw_query(
                query, None, ctx_info["video_proposal_feat"], ctx_info["video_feat"], ctx_info['video_mask'],
                cross=True)

        elif stage == 'hallucination':
            generate_query_feat = model.hallucination_encoder(model.hallucination_pos_embed(query_feat), query_mask.unsqueeze(1) if query_mask is not None else query_mask)
            ...
            hallucination_loss = model.hallucination_loss_visual(input_generate_query_feat,target_tokens,input_support_tokens_padding_mask,True)
            ...
            query = model.encode_query_generate(generate_query_feat, generate_query_mask)
            _clip_scale_scores, _frame_scale_scores = model.get_pred_from_raw_query_generate(
                query, None, ctx_info["video_proposal_feat"], ctx_info["video_feat"], ctx_info['video_mask'],cross=True)
...
def eval_epoch(model, val_video_dataset, val_text_dataset, opt, stage):
    model.eval()
    logger.info("Computing scores")

    context_info = compute_context_info(model, val_video_dataset, opt, stage)
    query_context_scores, global_query_context_scores, score_sum, query_metas = compute_query2ctx_info(model,val_text_dataset,opt,context_info,stage)
    video_metas = context_info['video_metas']
    v2t_gt, t2v_gt = get_gt(video_metas, query_metas)
    if stage == 'support':
        logging.info("Evaluation with support set captions:")
        print('Evaluation with support set captions:')
    elif stage == 'hallucination':
        logging.info("Evaluation with hallucination module:")
        print('Evaluation with hallucination module:')
...
def setup_model(opt):
    ...
    if opt.curriculum:
        ckpt_filepath = opt.ckpt_filepath + '_%s' % opt.support_set_number
    else:
...
def start_inference(opt=None):
    ...
    with torch.no_grad():
        score = eval_epoch(model, test_vid_dataset, test_text_dataset, opt, stage='hallucination')
